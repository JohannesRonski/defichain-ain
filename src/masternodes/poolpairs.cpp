...
void CPoolPairView::GetPoolPairForBlocks(DCT_ID const & poolId, uint32_t begin, uint32_t end, std::function<void(CPoolPair, uint32_t)> onPool) {
    if (begin >= end) {
        return;
    }
    // constexpr const uint32_t PRECISION = 10000;
    ...
    PoolReservesValue reserves;
    auto nextReserves = begin;
    auto itReserves = LowerBound<ByReservesHeight>(poolKey);
    ...
    for (auto height = begin; height < end;) {
        ...
        // adjust iterators to working height
        while (height >= nextReserves) {
            ReadValueMoveToNext(itReserves, poolId, reserves, nextReserves);
        }
        while (height >= nextPoolSwap) {
            poolSwapHeight = nextPoolSwap;
            ReadValueMoveToNext(itPoolSwap, poolId, poolSwap, nextPoolSwap);
        ...
        pool->rewards = customRewards;
        pool->rewardPct = poolReward;
        pool->rewardLoanPct = poolLoanReward;
    
        if (poolSwapHeight == height) {
            pool->blockCommissionA = poolSwap.blockCommissionA;
            pool->blockCommissionB = poolSwap.blockCommissionB;
        }
